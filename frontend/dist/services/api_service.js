const API_BASE = window.OS_SECURITY_API_BASE ||
    "http://127.0.0.1:5000";
const MAX_RETRIES = 3;
...
export async function checkAccess(payload) {
    const response = await fetchWithRetry(`${API_BASE}/check-access`, {
        method: "POST",
        headers: {
...
export async function getBackendConfig() {
    const response = await fetchWithRetry(`${API_BASE}/config`, { method: "GET" }, 1, HEALTH_TIMEOUT);
    if (!response.ok) {
...
export async function checkBackendHealth() {
    try {
        const response = await fetchWithRetry(`${API_BASE}/status`, { method: "GET" }, 1, HEALTH_TIMEOUT);
        return response.ok;
    }
    catch {
        return false;
